    ...
    IClaimAuditService auditService) : IClaimsApplicationService
{
    ...
    public IEnumerable<Claim> List(string actor)
    {
        ...
        return claimsStore.List().OrderByDescending(claim => claim.ServiceDate).ThenBy(claim => claim.ClaimId);
    }

    public IEnumerable<ClaimAuditLogEntry> GetListAccessAuditLogs()
        => auditService.GetListAccessAuditLogs();

    public IEnumerable<ClaimAuditLogEntry> GetAllAuditLogs()
        => auditService.GetAllAuditLogs();

    public Task<OperationResult<Claim>> GetById(string claimId, string actor)
    {
        var item = claimsStore.FindById(claimId);
        if (item is null)
        {
            return Task.FromResult(OperationResult<Claim>.Fail("Claim not found.", ErrorType.NotFound));
        }

        auditService.AddClaimRead(item.ClaimId, actor);
        return Task.FromResult(OperationResult<Claim>.Ok(item));
    }

    public Task<OperationResult<IEnumerable<ClaimAuditLogEntry>>> GetAuditLogs(string claimId)
    {
        var exists = claimsStore.FindById(claimId) is not null;
        if (!exists)
        {
            return Task.FromResult(OperationResult<IEnumerable<ClaimAuditLogEntry>>.Fail("Claim not found.", ErrorType.NotFound));
        }

        return Task.FromResult(OperationResult<IEnumerable<ClaimAuditLogEntry>>.Ok(auditService.GetAuditLogs(claimId)));
    }

    public ClaimStatusWorkflowModel GetStatusWorkflow() => workflowService.GetStatusWorkflow();

    public Task<OperationResult<Claim>> Create(Claim claim, string actor)
    {
        var validation = validationService.ValidateCreate(claim);
        if (!validation.Success || validation.Value is null)
        {
            return Task.FromResult(OperationResult<Claim>.Fail(validation.Error ?? "Validation failed.", validation.ErrorType ?? ErrorType.Validation));
        }

        claimsStore.Add(validation.Value);
        auditService.AddClaimCreated(validation.Value, actor);
        return Task.FromResult(OperationResult<Claim>.Ok(validation.Value));
    }

    public Task<OperationResult<Claim>> Update(string claimId, ClaimUpdateModel updates, string actor)
    {
    ...
    public Task<OperationResult<bool>> Delete(string claimId, string actor)
    {
        var deleted = claimsStore.Delete(claimId, out var removed);
        if (!deleted || removed is null)
        {
            return Task.FromResult(OperationResult<bool>.Fail("Claim not found.", ErrorType.NotFound));
        }
