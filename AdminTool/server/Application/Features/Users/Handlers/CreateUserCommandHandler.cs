...
{
    public async Task<OperationResult<User>> Handle(CreateUserCommand command, CancellationToken cancellationToken = default)
        => await usersService.Create(command.Name, command.Email, command.Role, command.Password, command.Actor);
}
